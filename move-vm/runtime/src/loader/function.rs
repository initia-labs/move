...
impl LoadedFunction {
    ...
    pub(crate) fn name_as_pretty_string(&self) -> String {
        match &self.owner {
            ...
            LoadedFunctionOwner::Module(m) => format!(
                "0x{}::{}::{}",
                m.arc_module().self_addr().to_hex(),
                m.arc_module().self_name().as_str(),
                self.function.name()
            ),
        }
    }
    ...
    pub(crate) fn get_resolver<'a>(&self, loader: &'a Loader) -> Resolver<'a> {
        match &self.owner {
            LoadedFunctionOwner::Module(module) => Resolver::for_module(loader, module.clone()),
            LoadedFunctionOwner::Script(script) => Resolver::for_script(loader, script.clone()),
        }
    }
}
