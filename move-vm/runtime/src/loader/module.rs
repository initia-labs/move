...
use move_vm_types::loaded_data::runtime_types::{
    Checksum, StructIdentifier, StructLayout, StructType, Type,
};
        ...
        let mut create = || {
            ...
            for (idx, struct_def) in module.struct_defs().iter().enumerate() {
                let definition_struct_type =
                    Arc::new(Self::make_struct_type(&module, struct_def, &struct_ids)?);
        ...
        let layout = match &struct_def.field_information {
            ...
            StructFieldInformation::Declared(fields) => {
                let fields: PartialVMResult<Vec<(Identifier, Type)>> = fields
                    .iter()
                    .map(|f| Self::make_field(module, f, struct_identifiers))
                    .collect();
